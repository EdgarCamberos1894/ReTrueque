...
export default function EditServicePage({ params }: { params: { id: number } }) {
  ...
  useEffect(() => {
    ...
    } else if (service && service.user.id !== userId) {
      router.replace(`/public/servicio/${service.id}`);
    }
  }, [router, service, token, userId]);

  return (
    <>
      <TopbarGeneral />
      {isLoading && <p className="mx-auto max-w-4xl px-4 py-10">Cargando servicio...</p>}
      {isError && <p className="mx-auto max-w-4xl px-4 py-10 text-red-700">No se pudo cargar el servicio.</p>}
      {service && service.user.id === userId && <ServiceForm service={service} />}
    </>
  );
}
